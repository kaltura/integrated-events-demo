...
import { KalturaUserModel } from "./kaltura-user.model";
...
export class KalturaMeetingExperienceModel {
    static async getKme(entry: any, userModel: KalturaUserModel): Promise<any[]> {
        // if entry type is not a room or a meeting entry - do not try to launch it.
        if (!entry || !entry.type || entry.type !== 'room.room' && (entry.adminTags && entry.adminTags.indexOf('_room') < 0)) {
            return [null, null];
        }
        ...
        const launchData: { [key: string]: string } = {
            ...
            user_id: userModel.user.id, // Internal authenticated user id in your system
            roles: userModel.isModerator ? KME_ROLES.moderator : KME_ROLES.viewer,
            lis_person_contact_email_primary: userModel.user.email, // User email
            lis_person_name_given: userModel.user.firstName,
            lis_person_name_family: userModel.user.lastName, // User last name
        ...
        launchData["integration_type"] = "kaltura";
